...
public partial class TableTop : TextureRect
{
	...
	private void SetMonthCard(float rotation, int row, int col, int anchor, int date)
	{
		...
		UpdateGrid(cardSelected, currentCard.Icon, cardSelected.Name, row, col, rotation, "month");
	}
	...
	private void SetDayCard(float rotation, int row, int col, int anchor, int date)
	{
		...
		AddChild(cardSelected);
		cardSelected.LockCard();
		GD.Print(cardSelected.Card);
		cardSelected.Cell[_gameData.CellDay].CellUnavailable();
		UpdateGrid(cardSelected, currentCard.Icon, cardSelected.Name, row, col, rotation, "day");
	}
	...
	public void DealCard()
	{
		...
		// will need to name the card when it is created
		// will need to increase the ordering to insure the card will be on top of the other cards
	...
	private void UpdateGrid(DragableCard cardSelected, Godot.Collections.Array<CardIcon> iconData, string group, int row, int col, float rotation, string type)
	{
